<template>
  <div class="calendar">
    ...
    <table cellpadding="5">
      <thead>
        <tr>
          <td v-for="(week, idx) in weeks" :key="idx" class="week">
            {{ week }}
          </td>
        </tr>
      </thead>
              ...
              :class="{
                red:
                  ...
                  ((child.isLunarFestival || child.isGregorianFestival) &&
                    lunar),
                'active-calendar-point': child.selected
              }"
            >
              {{ child.day }}
            </span>
    ...
    <div class="calendar-years" :class="{ show: yearsShow }">
      <span
        ...
        :class="{ 'active active-calendar-point': y === year }"
        >{{ y }}</span
      >
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    // 获取当前公历日期
    getCurrentDateFormat(format, begin, end) {
      // let date = param ? String(param) : null
      let date = null

      if (this.range) {
        date = begin && end ? [begin, end] : [this.rangeBegin, this.rangeEnd]
      } else if (this.multi) {
        date = begin || this.multiDays
      } else {
        // 单个选中
        date = begin
          ? String(begin)
          : String([
              ...
              this.zeroPad(this.day)
            ])
        date = m(new Date(date)).format(format)
      }
      return date
    },
    ...
    getLunarInfo(y, m, d) {
      let lunarInfo =
        !!y && !!m && !!d
          ...
          : calendar.solar2lunar(this.year, this.month + 1, this.day)
      ...
      if (
        this.festival.lunar[lunarInfo.lMonth + '-' + lunarInfo.lDay] !==
          undefined &&
        this.festival.gregorian[m + '-' + d] !== undefined
      ) {
        lunarValue = this.festival.gregorian[m + '-' + d]
        isLunarFestival = true
        isGregorianFestival = true
    ...
    changeYear() {
      ...
      this.yearsShow = true
      this.years = []
      for (let i = ~~this.year - 20; i < ~~this.year + 20; i++) {
        this.years.push(i)
      }
    },
    selectYear(value) {
...
.calendar-years > span + .calendar-years > span {
  margin-right: 2px;
}
